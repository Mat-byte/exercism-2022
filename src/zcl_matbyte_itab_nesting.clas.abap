CLASS zcl_matbyte_itab_nesting DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_matbyte_itab_nesting IMPLEMENTATION.

  METHOD perform_nesting.

    LOOP AT artists INTO DATA(artist).

      APPEND VALUE #( artist_id   = artist-artist_id
                      artist_name = artist-artist_name )
             TO nested_data ASSIGNING FIELD-SYMBOL(<nested_data>).

      LOOP AT albums INTO DATA(album) WHERE artist_id = artist-artist_id.

        APPEND VALUE #( album_id   = album-album_id
                        album_name = album-album_name )
            TO <nested_data>-albums ASSIGNING FIELD-SYMBOL(<nested_album>).

        LOOP AT songs INTO DATA(song) WHERE artist_id = artist-artist_id
                                        AND album_id  = album-album_id.

          APPEND VALUE #( song_id = song-song_id
