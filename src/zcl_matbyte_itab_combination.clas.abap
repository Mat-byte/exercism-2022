CLASS zcl_matbyte_itab_combination DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_matbyte_itab_combination IMPLEMENTATION.

  METHOD perform_combination.

    LOOP AT alphas INTO DATA(alpha).

      READ TABLE nums INTO DATA(num) INDEX sy-tabix.

      APPEND VALUE #( colx = |{ alpha-cola }{ num-col1 }|
